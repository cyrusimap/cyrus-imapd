...
static int
_parse_vcard(struct vparse_state *state, struct vparse_card *card, int only_one)
{
...
EXPORTED void
vparse_set_multival(struct vparse_state *state, const char *name, char split)
{
    switch (split) {
    case ';':
...
static void
_value_to_tgt(const char *value, struct vparse_target *tgt, unsigned flags)
{
 ...
 */
EXPORTED struct vparse_entry *
vparse_get_entry(struct vparse_card *card, const char *group, const char *name)
{
