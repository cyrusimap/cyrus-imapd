...
static void _setup_init(struct zs_open_data *setup, int flags)
{
    ...
    /* NOCRC only picks the checksum engine files get written with: no record
     * carries a checksum in format 4 (F-32) and an in-order file's single
     * checksum is verified on demand rather than on a read (F-26f), so there is
     * no verification left to skip - ZS_NOCSUM is retired and rejected. */
