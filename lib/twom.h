...
// there's 32 bits of space available - though not all flags have meaning in all
// contexts.
enum twom_flagspec {
    ...
    TWOM_NONBLOCKING = 1 << 4, /* When taking a lock, return immediately if the
    ...
    TWOM_IFEXIST = 1 << 12,    /* Only store if the record already exists (e.g.
                                  when deleting) */
    TWOM_FETCHNEXT = 1 << 13,  /* Return the record AFTER the given key */
    TWOM_SKIPROOT = 1 << 14, /* For foreach or cursor, skip the first record if
                                it matches exactly */
    TWOM_MVCC = 1 << 15, /* For cursor or transaction, operate in serializable
                            isolation (MVCC) mode */
