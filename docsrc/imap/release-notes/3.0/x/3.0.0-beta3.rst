...
*   The source repository has moved to :ref:`GitHub <github-guide>`, Bugzilla and Phabricator
    have been deprecated and their issue lists imported into GitHub.
...
*   Documentation cleanup: as part of the move to a single restructured text source for man
    pages and html format output, if you want to build the docs, you will need to 
    install the perl module ``Pod::POM::View::Restructured``. 

*   As part of the documentation cleanup, imap/admin/systemcommands/cyradm.rst and 
    imap/admin/configs/imapd.conf.rst are now both generated from perl/imap/IMAP/Shell.pm and
    lib/imapoptions respectively. Do not edit the restructured text versions! 
    ``make man`` and ``make html`` both now call ``make init`` which generates the rst files
    from the source.
