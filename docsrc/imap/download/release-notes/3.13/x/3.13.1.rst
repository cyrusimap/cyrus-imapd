...
* The ``admins_get_internal_seen`` configuration option has been added.  If
  set, the admin users will always get and set the same ``\Seen`` flags as the
  mailbox owner.
...
* The ``backupcyrusd`` protocol daemon has been added to facilitate rolling
  backups.
* The ``expire_by_savedate`` and ``expire_keep_flagged`` configuration options
  have been added to control the behavior of ``cyr_expire``.
* Time zone data is no longer included in iTIP messages.
* The IMAP PREVIEW extension will now only be advertised if
  the ``jmap_preview_annot`` configuration option is set and begins with
  ``/shared/``.
* Support for the IMAP ``UIDBATCHES`` extension has been added.
* ``cyrus.conf`` can now schedule ``EVENTS`` with cron-like syntax.
* The new ``sieve_mdn_original_recipient_header`` configuration option allows
  an install to get the original recipient from a header other than
  Original-Recipient.
* The new ``sieve_mdn_private`` configuration option helps prevent leaking
  usernames.
...
* ``lmtpd`` will no longer reset the session id after ``LHLO``.
