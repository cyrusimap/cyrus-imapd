...
#define CAPA_MIA	(1<<(CAPA_COMPRESS+6))
    ...
    CU_ASSERT_FATAL(n * sizeof(struct capa_t) +
		    sizeof(extra_capa) <= sizeof(test_prot.capa_cmd.capa));
    memcpy(&test_prot.capa_cmd.capa[n], extra_capa, sizeof(extra_capa));
...
static void test_oneline_caps(void)
{
    ...
    test_prot.capa_cmd.formatflags = CAPAF_MANY_PER_LINE;
    caps_common();
}
 ...
 */
static void server_auxprop_lookup(void *glob_context __attribute__((unused)),
...
static int init_sasl(int isclient)
{
	...
	static const struct sasl_callback server_cb[] = {
	    { SASL_CB_SERVER_USERDB_CHECKPASS, &server_checkpass, NULL },
	    { SASL_CB_LIST_END, NULL, NULL }
	};
