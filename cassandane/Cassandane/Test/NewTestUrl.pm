...
sub test_basic {
...

  {
    ...
    my $err = $@;
    $self->assert_matches(
      qr/\QCannot call ->url after ->unregister has been called\E/, $err,);
  }

  {
    ...
    my $err = $@;
    $self->assert_matches(
      qr/\QCannot call ->update after ->unregister has been called\E/, $err,);
  }
  ...
  {
    my $res = $lwp->post($plack_based_url->url,
      Content => encode_json({ good => 1 }),);
    $self->assert_str_equals('200', $res->code);

    my $json = decode_json($res->decoded_content);
    $self->assert_deep_equals({ good => "job" }, $json,);
  }

  {
    ...
    my $json = decode_json($res->decoded_content);
    $self->assert_deep_equals({ tough => "luck" }, $json,);
  }
}
