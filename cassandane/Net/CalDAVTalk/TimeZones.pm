...
#
# Generated: Mon Oct  6 01:29:39 2014
#
...
sub GetVTimeZone {
  ...
  my $StandardTzOffset = $RulesByZone->{$TimeZone}{StandardTzOffset};
  my $DaylightTzOffset = $RulesByZone->{$TimeZone}{DaylightTzOffset}  || $StandardTzOffset;

  my $VTimeZone = Data::ICal::Entry::TimeZone->new();
  $VTimeZone->add_property(tzid => $TimeZone);

  my $StandardVTimeZone = Data::ICal::Entry::TimeZone::Standard->new();

  $StandardVTimeZone->add_properties(
    dtstart      => '19700101T000000',
    tzoffsetfrom => $DaylightTzOffset,
    tzoffsetto   => $StandardTzOffset,
    ($RulesByZone->{$TimeZone}{StandardRRule}
      ? (rrule => $RulesByZone->{$TimeZone}{StandardRRule})
      : ()),
  );

  $VTimeZone->add_entry($StandardVTimeZone);

  if ($RulesByZone->{$TimeZone}{DaylightTzOffset}) {
    my $DaylightVTimeZone = Data::ICal::Entry::TimeZone::Daylight->new();

    $DaylightVTimeZone->add_properties(
      dtstart      => '19700101T000000',
      tzoffsetfrom => $StandardTzOffset,
      tzoffsetto   => $DaylightTzOffset,
      ($RulesByZone->{$TimeZone}{DaylightRRule}
        ? (rrule => $RulesByZone->{$TimeZone}{DaylightRRule})
        : ()),
    );

    $VTimeZone->add_entry($DaylightVTimeZone);
  }
