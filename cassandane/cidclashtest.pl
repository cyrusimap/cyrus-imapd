...
my $actual = check_messages($imap_store, $expected, "(after B)");

printf "generating message C\n" if $verbose;
$expected->{C} = make_message($gen, $imap_store,
			     "Message C",
			     references =>
...
check_messages($imap_store, $expected, "(after C)", $ElCid);
