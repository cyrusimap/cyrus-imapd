...
This appendix explains the use of this feature and its settings in full detail. 

.. note:: **Only on Mailbox Creation**

    Please note that the settings explained in this appendix only impact backend and partition selection during the creation of mailboxes. No automatic balancing of mailboxes by number, use or size during runtime is currently available, nor is any storage hierarchy concept. 

A.1. Guidelines
---------------

Depending on the situation, some modes will fit your needs more than the others. It mainly depends on whether you are building the platform from scratch, or adding new servers and partitions, and if you plan to create a lot of empty mailboxes in a short period of time.

Of course if the situation changes you should also adapt the selection mode.

If you plan to create a lot of empty mailboxes in a short period of time, the best selection mode is **random**. The other modes rely on the amount of freespace on backends and partitions, and thus would create most - if not all - the mailboxes at the same place, which will become an issue later with those mailboxes growth.
...
Selection modes configurable on frontend and backend do share the same principles. In each case, a list of available items are considered: those are the backends when creating the mailbox through a frontend, and the partitions when being on the backend. Depending on the configured selection mode, each item has an associated (disk) capacity and free space which will be used to select the most fitting one.

.. note:: **Unified mupdate**
    Please remember that when using a unified mupdate configuration, a server is both a frontend and a backend: you can create and access local or remote mailboxes. Since configuration settings are distinct for each role the server can take, you can apply different selection modes for each role.
...
**random**
    ...
    Choice is (pseudo-)random. Each item has the same probabilty of being selected.

**freespace-most**

    The item which has the most free space (counted in KiB units) is selected. That is the default selection mode.

    | Example A.1. Example of **freespace-most** Selection
    |
    | Suppose that the available items are:
    |
    | **item1** with 1000GiB of capacity, 400GiB being free (40% free space)
    | **item2** with 1000GiB of capacity, 600GiB being free (60% free space)
    | **item3** with 100GiB of capacity, 30GiB being free (30% free space)
    | **item4** with 100GiB of capacity, 70GiB being free (70% free space) 
    |
    | In that case **item2** will be selected as most fitting, since 600GiB of free space is the biggest of all.

**freespace-percent-most**

    The item which has the most percentage of free space is selected.

    | Example A.2. Example of freespace-percent-most Selection
    | 
    | In the same conditions than Example A.1, "Example of freespace-most Selection" **item4** would be selected as most fitting, since 70% of free space is the biggest of all.
...
Example A.3. Example of freespace-percent-weighted Selection
In the same conditions than Example A.1, “Example of freespace-most Selection”, the weight of each item would be::
...
.. note:: **Usage convergence**
    Using the freespace-percent-weighted mode, items percentage usages will converge towards 100%. So if they have different usages, those differences will stay and only really diminish upon reaching 100% of usage.
...
**freespace-percent-weighted-delta**
    ...
    | As for **freespace-percent-weighted**, a weight is associated to each item. It is computed as follows: (*percentage of freespace of item*) - (*lowest percentage of freespace of all items*) + 0.5
    |
...
.. note:: **Computed weight**
    The added 0.5 in items weight is so that selection gets smoother when all items usage percentages get close to each other.



Example A.4. Example of freespace-percent-weighted-delta Selection
In the same conditions than Example A.1, “Example of freespace-most Selection”, the weight of each item would be::

    40 - 30 + 0.5 = 10.5 for item1
    60 - 30 + 0.5 = 30.5 for item2
    30 - 30 + 0.5 = 0.5 for item3
    70 - 30 + 0.5 = 40.5 for item4 
...
A.3.1. What happens when two items are equal as most fitting ?
++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
A.3.2. What happens when two items are actually the same ?
++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
If two or more items share the same id when using the freespace-most or freespace-percent-most mode, only one of those items will actually be checked, as if the others were not configured. As for Section A.3.1, "What happens when two items are equal as most fitting ?", you may not know in advance which one will be. 
...
A.6.2.1. Backends Exclusion
"""""""""""""""""""""""""""

For details on exclusion based on disk usage, see Section A.4, "Items Exclusion".

Example of backend exclusion using serverlist_select_soft_usage_limit
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

The backend in Example A.7, “Example of backend representation on frontend” would be excluded using the **freespace-most** selection mode and setting **serverlist_select_soft_usage_limit** to 50 since in that mode it is represented by a disk usage of 55%. In other modes it would not be excluded, since it would be represented by a disk usage of 30%. 
