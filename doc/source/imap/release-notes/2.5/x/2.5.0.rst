...
Underscores in ``cmd`` Names in :manpage:`cyrus.conf(5)`
--------------------------------------------------------

Underscores (the ``_`` character) are no longer allowed in the
``START``, ``SERVICES`` and ``EVENTS`` sections of
:manpage:`cyrus.conf(5)`, as they interfere with configuration options
in :manpage:`imapd.conf(5)` being prefixed by service names and an
