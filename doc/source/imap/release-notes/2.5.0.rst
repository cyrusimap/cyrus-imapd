...
Something called "Index Namelock Release"
-----------------------------------------

deletes and cleanups work with long-running connections.
...
*   Number of Folders,
*   Number of Messages (per folder (hierarchy?)),
...
Cyrus IMAP now fully supports `RFC 5464`_, "*The IMAP METADATA
Extension*". This also means the support for the ANNOTATEMORE draft for
IMAP will ultimately be dropped.

*   Per message annotations

Replication-based XFER
----------------------
...
Default Change: ``unixhierarchysep``
------------------------------------

The default for the :manpage:`imapd.conf(5)` configuration option
``unixhierarchysep`` has changed from ``0`` (off) to ``1`` (on).

The setting controls whether to use the UNIX hierarchy separator
character '/', or use the netnews hierarchy separator '.'.

.. WARNING::

    Changing the option in a production run-time with clients will
    cause all clients to need to resynchronize their entire folder
    hierarchy at least.

    It is not uncommon for IMAP client software to not understand
    the change and fail.

Should you wish to preserve the current default, then set
``unixhierarchysep: 0`` in :manpage:`imapd.conf(5)` [#]_.
...
.. [#]
    ...
    You can set ``unixhierarchysep: 0`` in your current
    :manpage:`imapd.conf(5)` without consequence, before you upgrade to
    Cyrus IMAP 2.5.0.
