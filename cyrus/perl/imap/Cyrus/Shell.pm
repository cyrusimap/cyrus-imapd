...
# ugh.  ugh.  suck.  aieee.
my $use_rl = 'IMAP::Cyrus::DummyReadline';
{
...
sub shell {
  ...
  my $cyradm;
  if ($server ne '') {
    $cyradm = IMAP::Cyrus::Admin->new($server, $port)
...
sub _sc_server {
  ...
  if (!@nargv) {
    ...
    $$cyrref = IMAP::Cyrus::Admin->new($nargv[0])
...
package IMAP::Cyrus::DummyReadline;
use IO::File;
...
IMAP::Cyrus::Shell - Perl version of cyradm
