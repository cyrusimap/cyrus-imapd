                            ...
                            struct mdn_t *mdn, struct buf *msgbuf)
{
    ...
    if (mdn->client_final_rcpt) {
        const char *want = address_skip_type(mdn->client_final_rcpt);
...
static int jmap_mdn_send(struct jmap_req *req)
{
    ...
    /* RFC 9007, Section 2.1: one $mdnsent patch per MDN, no strays */
    if (!on_success) {
...
static json_t *_mdn_from_dn_fields(const char *base, size_t len)
{
    ...
    /* RFC 9007, Section 2.2 allows null when the original is unknown */
