    ...
    if (cdata->organizer) {
        ...
        if (strarray_contains_case(&schedule_addresses, cdata->organizer)) {
            ...
            if (_scheduling_enabled(txn, mailbox) && !is_draft)
                sched_defer(cal_ownerid, sched_userid, &schedule_addresses,
            ...
            if (_scheduling_enabled(txn, mailbox) && strarray_size(&schedule_addresses) && !is_draft)
                sched_defer(cal_ownerid, sched_userid, &schedule_addresses,
                            NULL, ical, NULL, cdata->dav.createdmodseq,
...
static int caldav_post_attach(struct transaction_t *txn, int rights)
{
    ...
    if (defer_sched && (ret == HTTP_CREATED || ret == HTTP_NO_CONTENT)) {
        ...
        sched_defer(cal_ownerid, sched_userid, &schedule_addresses,
                    defer_sched == DEFER_REQUEST ? sched_organizer : NULL,
                    oldical, ical, sched_cmodseq, SCHED_MECH_CALDAV,
                      ...
                      void *destdb, unsigned flags)
{
    ...
    if (!ret) {
        ...
        if (defer_sched && (ret == HTTP_CREATED || ret == HTTP_NO_CONTENT)) {
            ...
            sched_defer(cal_ownerid, sched_userid, &schedule_addresses,
                        defer_sched == DEFER_REQUEST ? organizer : NULL,
                        oldical, ical, cmodseq, SCHED_MECH_CALDAV,
                        mailbox_name(mailbox), resource, mailbox->i.last_uid);
            remove_etag = 1;
        }
