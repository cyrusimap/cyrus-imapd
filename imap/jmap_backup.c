                         ...
                         int is_destroy)
{
    ...
    if (strarray_contains_case(schedule_addresses, organizer)) {
        ...
        sched_defer(req->userid, req->userid, schedule_addresses, organizer,
                    oldical, ical, 0, SCHED_MECH_JMAP_RESTORE, NULL, NULL, 0);
    } else {
        ...
        if (!omit_reply && strarray_size(schedule_addresses))
            sched_defer(req->userid, req->userid, schedule_addresses, NULL,
