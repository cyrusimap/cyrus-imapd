...
static int jmap_calendarevent_get(struct jmap_req *req)
{
    ...
    if (json_array_size(get.ids)) {
        ...
        json_array_foreach(get.ids, i, jval) {
            const char *id = json_string_value(jval);
            struct jmap_caleventid *eid = jmap_caleventid_decode(id, db, NULL);
            const char *key = eid->ical_uid ? eid->ical_uid : eid->raw;
            ptrarray_t *eventids = hash_lookup(key, &eventids_by_uid);
            if (!eventids) {
                eventids = ptrarray_new();
                hash_insert(key, eventids, &eventids_by_uid);
            }
                                                          ...
                                                          struct caldav_data **cdata)
{
    ...
    return jmap_caleventid_decode(id, db, cdata);
}
...
static int jmap_calendarevent_set(struct jmap_req *req)
{
    ...
    json_array_foreach(set.destroy, index, juid) {
        ...
        struct caldav_data *cdata = NULL;
        eid = setcalendarevents_parse_id(req, id, db, &cdata);
        if (!eid || !eid->ical_uid) {
                                ...
                                json_t **set_err)
{
    ...
    /* Lookup event */
    struct caldav_data *cdata = NULL;
    struct jmap_caleventid *eid = jmap_caleventid_decode(src_id, src_db, &cdata);
    jmap_caleventid_free(&eid);
    if (!cdata || !cdata->dav.alive || !cdata->dav.rowid ||
