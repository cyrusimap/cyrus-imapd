...
static void merge_alarms(icalcomponent *comp, icalcomponent *alarms)
{
    ...
    if (alarms) {
                                                       ...
                                                       ICAL_VALARM_COMPONENT))
        {
            ...
            if (desc && *desc != '\0') {
                icalproperty *prop =
                    icalcomponent_get_first_property(myalarm,
    ...
    do {
        ...
        if (old) {
            ...
            int is_default =
                !!icalcomponent_get_x_property_by_name(old,
                                                       "X-JMAP-DEFAULT-ALARM");
            ...
            int is_acked =
                !!icalcomponent_get_first_property(old,
                                                   ICAL_ACKNOWLEDGED_PROPERTY);
                                          ...
                                          struct buf *buf)
{
    ...
    if (data.vpatch) {
             ...
             patch = nextpatch)
        {
            ...
            icalproperty *prop =
                icalcomponent_get_first_property(patch,
                                    ...
                                    json_t **errp)
{
    ...
    if (!(flags & DEFAULTALARMS_MIGRATE_KEEP_CALDAV_ALARMS)) {
        ...
        if (r) {
            ...
            json_object_set_new(caldav_alarm_err,
                                "withTime",
                                jmap_server_error(r));
        }
    ...
    while ((msg = mailbox_iter_step(mit))) {
        ...
        while (hash_iter_next(hit)) {
            ...
            if (strcmp(userid, ownerid)) {
                int rewrite = 0;
                migrate39_rewrite_peruser_data(mbox,
                                               userid,
                                               cdata,
                                               annotval,
                                               &defalarms,
                                               ...
                                               &buf);
                if (rewrite) {
                    r = mailbox_annotation_write(mbox,
                                                 record->uid,
                                                 PER_USER_CAL_DATA,
                                                 userid,
                                                 &buf);
