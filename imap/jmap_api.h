...
enum jmap_method_flags {
    ...
    JMAP_NO_ACCOUNTID = (1 << 3)  /* method is not account-scoped and takes no
                                     accountId argument (Core/echo) */
};
...
typedef struct jmap_prop_hash_table {
    const jmap_property_t *array;      // (sparse) array of all properties
    unsigned num_props;                // number of non-empty properties in array
    unsigned min_hash;                 // min hash value of non-empty properties
    unsigned max_hash;                 // max hash value of non-empty properties
    const jmap_property_t* (*lookup)   // lookup function into array
        (register const char *, register size_t);
} jmap_prop_hash_table_t;

typedef struct jmap_property_set {
    const jmap_prop_hash_table_t *map; // perfect hash table of all props
    const strarray_t wildcards;        // list of wildcard props, e.g. "foo*"
    const strarray_t always_get;       // list of JMAP_PROP_ALWAYS_GET props
    const strarray_t mandatory;        // list of JMAP_PROP_MANDATORY props
} jmap_property_set_t;
