...
// Non-standard iCalendar elements. The X-JMAP-* names shared with the
// CalDAV and iCalendar support layers live in ical_support.h.
...
static bool is_known_param(icalproperty *prop, icalparameter *param)
{
    ...
    case ICAL_ORGANIZER_PROPERTY:
        ...
        case ICAL_X_PARAMETER: {
            ...
            if (!strcasecmpsafe(JMAPICAL_XPARAM_DTSTAMP, xname)
                || !strcasecmpsafe(JMAPICAL_XPARAM_SEQUENCE, xname))
            {
                return true;
            }
            return false;
...
static bool is_known_prop(icalcomponent *comp, icalproperty *prop)
{
    ...
    case ICAL_VTODO_COMPONENT:
        ...
        default:
            ...
            if (myicalproperty_has_name(prop, JMAPICAL_XPROP_USEDEFAULTALERTS))
                return true;
            // XXX quirk: these got set in the former implementation
            if (myicalproperty_has_name(prop, JMAPICAL_XPROP_ID))
                return true;
            if (myicalproperty_has_name(prop, JMAPICAL_XPROP_HIDEATTENDEES))
                return true;
            if (myicalproperty_has_name(prop, JMAPICAL_XPROP_MAYINVITEOTHERS))
                return true;
            if (myicalproperty_has_name(prop, JMAPICAL_XPROP_MAYINVITESELF))
                return true;
            if (myicalproperty_has_name(prop, JMAPICAL_XPROP_PRIVACY))
                return true;
            if (myicalproperty_has_name(prop, JMAPICAL_XPROP_SENTBY))
                return true;
...
static bool is_usable_uid(const char *str)
{
    ...
    // Check contents and character count.
...
static bool geouri_parse(const char *uri, struct geouri *geouri)
{
    ...
    int n_coords;
    for (n_coords = 0; n_coords < 3; n_coords++) {
        if (n_coords) {
            if (*str != ',')
                break;
            str++;
        ...
        geouri->coords[n_coords] = xstrndup(str, num - str);
        str = num;
    }
...
static struct icalgeotype geouri_to_icalgeo(const char *uri, bool *is_lossy)
{
    ...
    *is_lossy = geouri.coords[2] || geouri.p;
    ...
    if (strlcpy(icalgeo.lat, geouri.coords[0], ICAL_GEO_LEN) >= ICAL_GEO_LEN) {
        *is_lossy = true;
    }

    if (strlcpy(icalgeo.lon, geouri.coords[1], ICAL_GEO_LEN) >= ICAL_GEO_LEN) {
        *is_lossy = true;
    }
                                                ...
                                                icaltimezone *tz_until)
{
    ...
    if (ival > 1) buf_printf(&buf, ";INTERVAL=%" JSON_INTEGER_FORMAT, ival);
    ...
    if (json_array_size(jbyday)) {
        ...
        json_array_foreach(jbyday, i, jnday)
        {
            ...
            if (i) buf_putc(&buf, ',');
            if (n) buf_printf(&buf, "%" JSON_INTEGER_FORMAT, n);
        ...
        if (json_array_size(jbyx)) {
            ...
            json_array_foreach(jbyx, j, jval)
            {
                ...
                if (json_is_string(jval)) {
                    ...
                    buf_printf(&buf, "%" JSON_INTEGER_FORMAT, json_integer_value(jval));
                }
            }
        }
    }
    ...
    if (count > 0 && !json_object_get(jrrule, "until"))
        buf_printf(&buf, ";COUNT=%" JSON_INTEGER_FORMAT, count);
                               ...
                               icaltimezone *tz_until)
{
    ...
    case ICAL_MONDAY_WEEKDAY:
        break;
    default: {
        struct buf day = BUF_INITIALIZER;
        buf_setcstr(&day, icalrecur_weekday_to_string(rrule->week_start));
        json_object_set_new(jrrule,
                            "firstDayOfWeek",
                            json_string(buf_lcase(&day)));
        buf_free(&day);
    }
    }
         ...
         byrule++)
    {
        ...
        for (short i = 0; i < rrule->by[byrule].size; i++) {
            ...
            case ICAL_BY_DAY: {
                ...
                struct buf day = BUF_INITIALIZER;
                buf_setcstr(&day, icalrecur_weekday_to_string(wday));
                                        ...
                                        enum get_ical_flags flags)
{
    ...
    if (!ctx->cfg.use_icalendar_convprops) {
        if (want_kind != ICAL_ANY_COMPONENT && (flags & GET_ICAL_CREATE)) {
            return icalcomponent_new(want_kind);
        }
        return NULL;
    }
...
static bool prop_has_jsid(icalproperty *prop)
{
    return myicalproperty_get_parameter_by_name(prop, PARAM_JSID) ||
           myicalproperty_get_parameter_by_name(prop, JMAPICAL_XPROP_ID);
}
                                  ...
                                  struct buf *buf)
{
    ...
    if (!jsid) {
        param = myicalproperty_get_parameter_by_name(prop, JMAPICAL_XPROP_ID);
...
static bool comp_has_jsid(icalcomponent *prop)
{
    return myicalcomponent_get_property_by_name(prop, PROP_JSID) ||
           myicalcomponent_get_property_by_name(prop, JMAPICAL_XPROP_ID);
}
                                  ...
                                  struct buf *buf)
{
    ...
    if (!prop)
        prop = myicalcomponent_get_property_by_name(comp, JMAPICAL_XPROP_ID);
    if (prop) {
    ...
    json_object_foreach(jalerts, key, jalert)
    {
            ...
            && was_action != ICAL_ACTION_EMAIL)
        {
            ...
            if (!myicalcomponent_get_property(valarm, ICAL_DESCRIPTION_PROPERTY)) {
                const char *s = ctx->cfg.displayalert_default_description;
                if (!s) s = "Reminder";
                icalcomponent_add_property(valarm,
                                           icalproperty_new_description(s));
            }
        }
        ...
        if (caladdr) {
            ...
            if (attendee) {
                ...
                if (json_is_integer(jval) && json_integer_value(jval) >= 0) {
                    ...
                    icalparameter_set_iana_name(param, JMAPICAL_XPARAM_SEQUENCE);
                    icalproperty_add_parameter(attendee, param);
                    buf_free(&buf);
                }
                ...
                if (json_is_string(jval)) {
                    ...
                    if (!icaltime_is_null_time(t)) {
                        ...
                        icalparameter_set_iana_name(param, JMAPICAL_XPARAM_DTSTAMP);
                        icalproperty_add_parameter(attendee, param);
                        free(tmp);
                    }
                }
                              ...
                              icalcomponent *comp)
{
    ...
    icalproperty *dtstart = jobj_get_icalprop(ctx, jobj, "start",
            ICAL_DTSTART_PROPERTY, GET_ICAL_CREATE);
    ...
    if (!icaltime_is_null_time(due)) {
        icalproperty *prop = jobj_get_icalprop(ctx, jobj, "due",
                ICAL_DUE_PROPERTY, GET_ICAL_CREATE);
    ...
    if (!icaltime_is_null_time(recurid)) {
        icalproperty *prop =
            jobj_get_icalprop(ctx, jobj, "recurrenceId",
                    ICAL_RECURRENCEID_PROPERTY, GET_ICAL_CREATE);
    ...
    if (!icaldurationtype_is_null_duration(duration)) {
        ...
        icalproperty *dtend = jobj_get_icalprop(
            ctx, jobj, "duration", ICAL_DTEND_PROPERTY, 0);
...
static void sanitize_override_patch(json_t *jpatch)
{
    ...
    json_object_foreach(jpatch, pkey, pval) {
        for (int i = 0; skip_prefixes[i]; i++) {
            ...
            if (!strncmp(pkey, prefix, prefix_len) &&
                    (pkey[prefix_len] == '/' || !pkey[prefix_len])) {
                           ...
                           struct jmap_parser *parser, json_t *jentry)
{
    ...
    json_object_foreach(jentry, key, jval)
    {
        if (!strcmp("@type", key)) {
            ...
            const char *s = json_string_value(jval);
            if (!s || !ical_is_valid_color(s, false/*allow_alpha*/)) {
                jmap_parser_invalid(parser, key);
            }
        }
...
static void participant_from_icalprop(icalproperty *prop, json_t *jpart)
{
        ...
        prop, ICAL_ANY_PARAMETER, param, param_iter)
    {
        ...
        else if (myicalparameter_has_name(param, JMAPICAL_XPARAM_SEQUENCE)) {
        ...
        else if (myicalparameter_has_name(param, JMAPICAL_XPARAM_DTSTAMP)) {
            const char *xval = icalparameter_get_value_as_string(param);
            if (xval) {
                            ...
                            ptrarray_t *overrides)
{
    ...
    if ((prop = myicalcomponent_get_property_by_name(
             comp, JMAPICAL_XPROP_USEDEFAULTALERTS)) ||
        // XXX quirk: our previous jscalendar draft implementation
        // erroneously used the X-APPLE-DEFAULT-ALARM annotation in
    ...
    if ((prop = myicalcomponent_get_property_by_name(
             comp, JMAPICAL_XPROP_HIDEATTENDEES))) {
        const char *v = icalproperty_get_value_as_string(prop);
        if (!strcasecmpsafe(v, "TRUE"))
    ...
    if ((prop = myicalcomponent_get_property_by_name(
             comp, JMAPICAL_XPROP_MAYINVITEOTHERS))) {
        const char *v = icalproperty_get_value_as_string(prop);
        if (!strcasecmpsafe(v, "TRUE"))
    ...
    if ((prop = myicalcomponent_get_property_by_name(
             comp, JMAPICAL_XPROP_MAYINVITESELF))) {
        const char *v = icalproperty_get_value_as_string(prop);
        if (!strcasecmpsafe(v, "TRUE"))
...
HIDDEN json_t *jscal_from_ical(jscal_cfg_t *cfg, icalcomponent *ical)
{
