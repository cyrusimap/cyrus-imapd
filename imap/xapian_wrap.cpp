                              ...
                              int (*cb)(void *data, size_t n, void *rock), void *rock)
{
    ...
    try {
        ...
        if (size) data = xzmalloc(size * SEARCH_GUIDREP_SIZE);
        for (Xapian::MSetIterator i = matches.begin() ; i != matches.end() ; ++i) {
            ...
            char *entry = (char *) data + (SEARCH_GUIDREP_SIZE * n);
            memcpy(entry, cstr+3, MESSAGE_GUIDREP_SIZE);
            entry[MESSAGE_GUIDREP_SIZE] = '\0';
            ++n;
        }
    }
    catch (const Xapian::Error &err) {
