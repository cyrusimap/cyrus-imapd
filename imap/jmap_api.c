                                                 ...
                                                 jmap_property_set_t *prop_set)
{
    ...
    for (int i = 0; i < ptrarray_size(&prop_set->wildcards); i++) {
        prop = ptrarray_nth(&prop_set->wildcards, i);
        if (!strncmp(name, prop->name, strlen(prop->name)-1)) {
            return prop;
        }
    }
                           ...
                           json_t **err)
{
    ...
    if (get->props == NULL) {
        ...
        int nvalid = valid_props->map->num_props;
        ...
        for (int i = 0; i < ptrarray_size(&valid_props->always_get); i++) {
            const jmap_property_t *prop =
                ptrarray_nth(&valid_props->always_get, i);
                                    ...
                                    json_t **err)
{
    ...
    if (!id && valid_props) {
        ...
        for (int i = 0; i < ptrarray_size(&valid_props->mandatory); i++) {
            prop = ptrarray_nth(&valid_props->mandatory, i);
