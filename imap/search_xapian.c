...
static uint8_t is_indexed(search_text_receiver_t *rx, message_t *msg)
{
    ...
    if (tr->mode == XAPIAN_DBW_CONVINDEXED) {
         ...
         * whose documents are not actually in Xapian (gap-filled ranges, an
         * interrupted run, or an expunged duplicate), which would silently
         * drop a live message from the index.  Confirm any positive against
         * Xapian, which is authoritative.  A negative needs no such
         * verification; the db is opened here lazily, only to verify a
         * positive. */
        if (ret) {
            if (!tr->dbw) {
